    ...
    <header className="topbar">
      ...
      <div className="search-wrap">
        ...
        {focused && q.trim().length >= 2 && <div className="search-preview glass">
          ...
          {previewItems.map((item) => <button key={`${item.type}-${item.id}`} onMouseDown={(e) => e.preventDefault()} onClick={() => { setFocused(false); onOpenItem(item); }}>{item.poster ? <img src={item.poster} alt="" /> : <div className="mini-poster" />}<span><strong>{item.name}</strong><small>{item.type === "series" ? "Serial" : "Film"}</small></span></button>)}
      ...
      <div className="menu-wrap"><button className="icon" onClick={() => setOpen(!open)}><Settings /></button>{open && <div className="menu glass"><div><User size={16} /> {profile?.username || profile?.display_name || "Konto"}</div>{profile?.role === "admin" && <button onClick={onAdmin}><KeyRound size={16} /> Panel admina</button>}<button onClick={onLogout}><LogOut size={16} /> Wyloguj</button></div>}</div>
    </header>
  );
}
...
function Hero({ items, hasLibraries, onOpen }) {
  ...
  return <section className="hero" style={{ backgroundImage: bg ? `linear-gradient(90deg, #0b0b0f 0%, rgba(11,11,15,.72) 45%, rgba(11,11,15,.18)), url(${bg})` : undefined }}><div className="hero-copy"><div className="eyebrow">Proponowane</div><h1>{item.name}</h1><p>{item.description || "Materiał z biblioteki addonu."}</p><button className="primary" onClick={() => onOpen(item)}><Play size={18} /> Otwórz</button></div><div className="hero-dots">{list.map((_, i) => <button key={i} className={i === index ? "active" : ""} onClick={() => setIndex(i)} />)}</div></section>;
}
...
function ResultsPage({ query, results, onBack, onOpen }) {
  ...
  return <main className="results-page"><button className="link back" onClick={onBack}>← Wróć</button><h1>Wyniki dla „{query}”</h1><section className="result-section"><h2>Filmy</h2>{movies.length ? <div className="result-grid">{movies.map((item) => <button className="poster" key={`movie-${item.id}`} onClick={() => onOpen({ ...item, type: "movie" })}>{item.poster ? <img src={item.poster} alt="" /> : <div className="poster-fallback">{item.name}</div>}<strong>{item.name}</strong></button>)}</div> : <p>Brak filmów.</p>}</section><section className="result-section"><h2>Seriale</h2>{series.length ? <div className="result-grid">{series.map((item) => <button className="poster" key={`series-${item.id}`} onClick={() => onOpen({ ...item, type: "series" })}>{item.poster ? <img src={item.poster} alt="" /> : <div className="poster-fallback">{item.name}</div>}<strong>{item.name}</strong></button>)}</div> : <p>Brak seriali.</p>}</section></main>;
}
...
function Details({ item, history, onBack, onPlay }) {
  ...
  const nextEpisode = useMemo(() => {
    ...
    return last.duration_seconds > 0 && last.duration_seconds - last.position_seconds <= 300 ? sorted[idx + 1] || sorted[idx] : sorted[idx];
  }, [videos, last]);
...
function Player({ item, initialStreams, profile, onClose, onLimitUpdate }) {
  ...
  function updateEmbeddedTracks() { const tracks = videoRef.current?.textTracks; if (!tracks) return; const list = Array.from(tracks).map((track, index) => ({ index, label: track.label || languageName(track.language) || `Ścieżka ${index + 1}`, language: track.language })).filter((track) => !String(track.label || "").startsWith("Addon •")); setEmbeddedTracks(list); }
  ...
  function applyAudioChoice(value) { const tracks = videoRef.current?.audioTracks; if (!tracks || typeof tracks.length !== "number") return; const index = value === "auto" ? -1 : Number(String(value).replace("audio:", "")); for (let i = 0; i < tracks.length; i += 1) tracks[i].enabled = i === index; setAudioTracks(Array.from(tracks).map((track, i) => ({ index: i, id: track.id, label: track.label || `Audio ${i + 1}`, language: track.language || "", enabled: track.enabled }))); }
  ...
  async function fetchStreamsOnce(active) { const data = await api(`/streams/${item.type || "movie"}/${encodeURIComponent(item.id)}`); if (!active()) return []; const nextStreams = data.streams || []; setStreams(nextStreams); setStreamIndex(0); if (nextStreams.length > 0) setTimeout(tryAutoplay, 150); return nextStreams; }
  ...
  useEffect(() => {
    ...
    async function runStreamSearch() {
      try {
        ...
        for (let remaining = 30; remaining > 0; remaining -= 1) { if (!isActive()) return; setRetryCountdown(remaining); await sleep(1000); }
        if (!isActive()) return;
        setRetryCountdown(0); setLoading(true);
      ...
      finally { if (isActive()) { setLoading(false); setRetryCountdown(null); } }
    }
    runStreamSearch();
    api(`/subtitles/${item.type || "movie"}/${encodeURIComponent(item.id)}`).then((data) => active && setSubtitles(data.subtitles || [])).catch(() => {});
    api("/playback/start", { method: "POST", body: JSON.stringify({ content_type: item.type || "movie", content_id: item.id, title: item.name, season: item.season, episode: item.episode }) }).then((data) => { if (profile?.role === "guest" && data.remaining !== null) { setNotice(`Pozostało ci ${data.remaining} z ${profile.limit} limitu odtworzonych filmów`); onLimitUpdate?.(data.remaining); setTimeout(() => setNotice(""), 4500); } }).catch((err) => setNotice(err.message));
    return () => { active = false; };
  }, [item.id, item.type]);
  ...
  useEffect(() => { const tracks = videoRef.current?.textTracks; if (!tracks) return; for (let i = 0; i < tracks.length; i += 1) tracks[i].mode = "disabled"; if (subtitleChoice.startsWith("embedded:")) { const index = Number(subtitleChoice.split(":")[1]); if (tracks[index]) tracks[index].mode = "showing"; } if (subtitleChoice.startsWith("addon:")) { const idx = Number(subtitleChoice.split(":")[1]); const label = addonSubtitleLabel(subtitles[idx], idx); const track = Array.from(tracks).find((candidate) => candidate.label === label); if (track) track.mode = "showing"; } }, [subtitleChoice, subtitles.length, embeddedTracks.length]);
...
function App() {
  ...
  const [libraries, setLibraries] = useState(cached.libraries || []), [settings, setSettings] = useState(cached.settings || {}), [history, setHistory] = useState(cached.history || []);
  const [screen, setScreen] = useState(savedUi.screen || "home"), [selected, setSelected] = useState(savedUi.selected || null), [playerData, setPlayerData] = useState(savedUi.playerData || null);
  ...
  async function refresh() { const me = await api("/me"); setProfile({ ...getProfile(), ...me }); const data = await api("/catalogs"); const nextHistory = (await api("/playback/history")).history || []; setLibraries(data.libraries || []); setSettings(data.settings || {}); setHistory(nextHistory); writeJson(CACHE_KEY, { libraries: data.libraries || [], settings: data.settings || {}, history: nextHistory, updated_at: Date.now() }); }
  useEffect(() => { if (profile) refresh().catch(() => { if (!libraries.length) { clearSession(); setProfile(null); } }); }, []);
  useEffect(() => { writeJson(UI_KEY, { screen, selected, playerData }); }, [screen, selected, playerData]);
