  ...
  async function submit(event) {
    ...
    try {
      if (mode === "guest") {
        ...
        setSession(data.token, { role: "guest", ...data.guest }); onLogin(); return;
      }
      ...
      setSession(data.token, data.user); onLogin();
...
function TopBar({ profile, onLogout, onSearch, onSearchPreview, searchPreview, onOpenItem, onAdmin }) {
  ...
  useEffect(() => { const query = q.trim(); if (query.length < 2) return; const id = setTimeout(() => onSearchPreview(query), 1000); return () => clearTimeout(id); }, [q]);
  ...
  return <header className="topbar"><div className="brand"><Clapperboard /> <span>Player</span></div><div className="search-wrap"><form className="search" onSubmit={submit}><Search size={18} /><input placeholder="Szukaj filmu lub serialu..." value={q} onFocus={() => setFocused(true)} onChange={(e) => setQ(e.target.value)} /></form>{focused && q.trim().length >= 2 && <div className="search-preview glass">{searchPreview?.loading && <div className="preview-loading"><Loader2 className="spin" /> Szukam...</div>}{!searchPreview?.loading && previewItems.length === 0 && <div className="preview-empty">Brak wyników</div>}{previewItems.map((item) => <button key={`${item.type}-${item.id}`} onMouseDown={(e) => e.preventDefault()} onClick={() => { setFocused(false); onOpenItem(item); }}>{item.poster ? <img src={item.poster} alt="" /> : <div className="mini-poster" />}<span><strong>{item.name}</strong><small>{item.type === "series" ? "Serial" : "Film"}</small></span></button>)}{previewItems.length > 0 && <button className="all-results" onMouseDown={(e) => e.preventDefault()} onClick={() => { setFocused(false); onSearch(q.trim()); }}>Pokaż wszystkie wyniki</button>}</div>}</div>{profile?.role === "guest" && <div className="limit">Limit: {profile.remaining ?? "?"}/{profile.limit ?? "?"}</div>}<div className="menu-wrap"><button className="icon" onClick={() => setOpen(!open)}><Settings /></button>{open && <div className="menu glass"><div><User size={16} /> {profile?.username || profile?.display_name || "Konto"}</div>{profile?.role === "admin" && <button onClick={onAdmin}><KeyRound size={16} /> Panel admina</button>}<button onClick={onLogout}><LogOut size={16} /> Wyloguj</button></div>}</div></header>;
}

function Hero({ items, hasLibraries, onOpen }) {
  const [index, setIndex] = useState(0);
  const list = (items || []).slice(0, 10);
  useEffect(() => { if (list.length <= 1) return; const id = setInterval(() => setIndex((v) => (v + 1) % list.length), 15000); return () => clearInterval(id); }, [list.length]);
...
function Details({ item, history, onBack, onPlay }) {
  ...
  useEffect(() => { let active = true; setMeta(item); api(`/meta/${item.type || itemType(item)}/${encodeURIComponent(item.id)}`).then((data) => active && setMeta({ ...item, ...data.meta, type: item.type || itemType(data.meta) })).catch(() => {}); return () => { active = false; }; }, [item.id]);
  ...
  const nextEpisode = useMemo(() => { if (!videos.length) return null; const sorted = [...videos].sort((a, b) => (a.season ?? 1) - (b.season ?? 1) || (a.episode ?? a.number ?? 1) - (b.episode ?? b.number ?? 1)); if (!last) return sorted[0]; const idx = sorted.findIndex((ep) => ep.id === last.content_id); if (idx < 0) return sorted[0]; const almostFinished = last.duration_seconds > 0 && last.duration_seconds - last.position_seconds <= 300; return almostFinished ? sorted[idx + 1] || sorted[idx] : sorted[idx]; }, [videos, last]);
...
function Player({ item, initialStreams, profile, onClose, onLimitUpdate }) {
  ...
  useEffect(() => {
    ...
    if (subtitleChoice.startsWith("embedded:")) {
      const index = Number(subtitleChoice.split(":")[1]);
      if (tracks[index]) tracks[index].mode = "showing";
    }
    if (subtitleChoice.startsWith("addon:")) {
      const idx = Number(subtitleChoice.split(":")[1]);
      const label = addonSubtitleLabel(subtitles[idx], idx);
      const track = Array.from(tracks).find((candidate) => candidate.label === label);
      if (track) track.mode = "showing";
    }
  }, [subtitleChoice, subtitles.length, embeddedTracks.length]);
...
function App() {
  ...
  async function refresh() {
    ...
    const nextHistory = (await api("/playback/history")).history || [];
    setLibraries(data.libraries || []);
    setSettings(data.settings || {});
    setHistory(nextHistory);
    writeJson(CACHE_KEY, { libraries: data.libraries || [], settings: data.settings || {}, history: nextHistory, updated_at: Date.now() });
  }
